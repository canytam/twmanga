...
# Add cleanup for temporary files if interrupted
import signal
...
async def process_pdf_batch(session, group, batch_number, folder_name, image_semaphore, total_batches, title):
    logger = logging.getLogger()
    batch_desc = f"Batch {batch_number}/{total_batches}"
    logger.info(f"[{batch_desc}] Processing batch with {len(group)} chapters")
    
    # Sanitize title for filename use
    safe_title = sanitize_filename(title)
    
    # Collect all image URLs and chapter data
    chapters_data = []
    total_images = 0
    for chapter in group:
        ...
        try:
            html = await fetch_page(session, chapter_link)
            soup = BeautifulSoup(html, 'html.parser')
            image_urls = []
            for tag in soup.find_all(['a', 'img']):
                url = tag.get('href') or tag.get('src')
                if url:
                    absolute_url = urljoin(chapter_link, url)
                    if urlparse(absolute_url).path.lower().endswith(('.jpg', '.jpeg')):
                        image_urls.append(absolute_url)
            if not image_urls:
                logger.warning(f"[{batch_desc}] No images found in chapter: {chapter_desc}")
                continue
            ...
            logger.error(f"[{batch_desc}] Failed to process chapter {chapter_desc}: {str(e)}")
            continue
    
    if total_images == 0:
        logger.warning(f"[{batch_desc}] No images found in batch")
        return None
    
    # Download images
    with tempfile.TemporaryDirectory() as tmpdir:
        all_images = []
        with tqdm_asyncio(total=total_images, desc=f"Batch {batch_number}") as pbar:
            tasks = []
...
async def main_async(book_id, num_pdfs):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            folder_name = f"{title}_{book_id}_{num_pdfs}PDFs"
            folder_name = "".join(c if c.isalnum() or c in "_-" else "_" for c in folder_name)[:100]
            ...
            logging.info(f"Generating {num_pdfs} PDF files")
            ...
            logging.info(f"Split into {total_batches} batches")
            ...
            for batch_start in range(0, total_batches, MAX_CONCURRENT_BOOKS):
                ...
                for idx, group in enumerate(current_groups, start=batch_start):
                    tasks.append(
                        process_pdf_batch(session, group, idx+1, folder_name, image_semaphore, total_batches, title)
                    )
                results = await asyncio.gather(*tasks)
                
                for result in results:
                    if result:
                        pdf_name, chapters_in_batch = result
                        pdf_entries.append( (pdf_name, chapters_in_batch) )
                        success += 1
                    else:
...
<body>
    ...
    <ul>
""")
                    for pdf_name, chapters_in_batch in pdf_entries:
                        chapter_list = ", ".join(chapters_in_batch)
                        f.write(f'        <li><a href="{pdf_name}">{pdf_name}</a><div class="chapters">Chapters: {chapter_list}</div></li>\n')
                    f.write("""    </ul>
</body>
</html>""")
            ...
            # Final report
            logging.info("\nPROCESSING SUMMARY:")
            logging.info(f"Requested PDFs: {num_pdfs}")
            logging.info(f"Total chapters found: {len(sorted_chapters)}")
            logging.info(f"Total batches created: {total_batches}")
            logging.info(f"Successfully processed batches: {success}")
